...
/*   Updated: 2022/11/26 17:51:09 by almirand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_texture_image(t_window *wndw, unsigned char	*txture, char	*path, t_img	*img);
...
int	init_window(t_content *content)
{
	...
	wndw.img.addr = (unsigned char *)mlx_get_data_addr(wndw.img.image, &wndw.img.bpp, \
...
int	key_hook(int key, t_window	*wndw)
{
	if (key == W_U)
	{
		if (!wndw->content->map[(int)(wndw->pos_y)][(int)(wndw->pos_x + wndw->dir_x * wndw->spd_move)])
			wndw->pos_x += wndw->dir_x * wndw->spd_move;
		if (!wndw->content->map[(int)(wndw->pos_y + wndw->dir_y * wndw->spd_move)][(int)wndw->pos_x])
	...
	if (key == D_R)
	{
		...
		wndw->dir_x = wndw->dir_x * cos(-wndw->spd_cam) - wndw->dir_y * sin(-wndw->spd_cam);
		wndw->dir_y = wndw->aux_dirx * sin(-wndw->spd_cam) + wndw->dir_y * cos(-wndw->spd_cam);
		wndw->aux_planex = wndw->plane_x;
		wndw->plane_x = wndw->plane_x * cos(-wndw->spd_cam) - wndw->plane_y * sin(-wndw->spd_cam);
		wndw->plane_y = wndw->aux_planex * sin(-wndw->spd_cam) + wndw->plane_y * cos(-wndw->spd_cam);
	}
